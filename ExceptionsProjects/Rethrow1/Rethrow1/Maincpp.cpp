...
void processPositive(int num)
{
	...
	if (num >= 0)
	{
		cout << "Good job! You passed in a positive number to processPositive" << endl;
	}
	else
	{
...
void doSomething(int num)
{
	...
	catch (const invalid_argument &err)
	{
		...
		throw; // rethrow the invalid_arguement exception
	}
	catch (const out_of_range &err)
	{
		cout << "doSomething says the number is too big!" << endl;
		throw; // rethrow the out_of_range exception
	}
}